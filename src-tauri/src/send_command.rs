...
pub async fn send_start(
    ...
    for index in 0..thread_count {
        ...
        join_set.spawn(async move {
            let mut client: Box<dyn Client + Send> = if protocol == 0 {
                Box::new(UdpClient::new(value).await.expect("udp客户端创建失败"))
            } else {
                Box::new(TcpClient::new(value).await.expect("tcp客户端创建失败"))
            };
...
#[tauri::command]
pub async fn send_stop() -> crate::Result<()> {
    RUNNING.store(false, Ordering::SeqCst);
    Ok(())
}
lazy_static! {
...
fn build_packet(index: i32) -> Vec<u8> {
    ...
    for i in 0..26 {
        ...
        let aoa_tag = AoaTag {
            ...
            crc: 0x00,
            df_field: [
                0x2F, 0x61, 0xAC, 0xCC, 0x27, 0x45, 0x67, 0xF7, 0xDB, 0x34, 0xC4, 0x03, 0x8E, 0x5C,
                0x0B, 0xAA, 0x97, 0x30, 0x56, 0xE6,
            ],
            rssi: 0x04,
        };
